...
(defn graph [!view event-series]
  (fn []
    (let [{:keys [zoom max-buckets-at-zoom-1]} @!view
          event-types (sort (into #{} (map group-by-event-type event-series)))
          aggregated-event-series (aggregate-and-group event-series max-buckets-at-zoom-1 zoom group-by-event-type)] ; zooming in aggregates into more buckets
      [:div
       [:input {:type     :button
                :value    "+"
                :on-click #(swap! !view update :zoom inc)}]
       [:span (:zoom @!view)]
       [:input {:type     :button
                :value    "-"
                :on-click #(swap! !view update :zoom dec)}]
