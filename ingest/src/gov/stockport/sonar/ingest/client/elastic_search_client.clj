...
(defn bulk-index-new [{:keys [file valid-event] :as feed}]
  (let [index-name (str/join "-" ["events" (esname (::es/event-source (first valid-event))) (fs/mod-time file)])]
    (bulk-index (fn [_] index-name) valid-event)
